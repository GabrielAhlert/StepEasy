...
pub fn show(app: &mut App, ui: &mut egui::Ui) {
    ...
    egui::CentralPanel::default().show(ui, |ui| {
        ui.vertical_centered(|ui| {
            ...
            ui.add_space(10.0);
            ui.label(
                RichText::new(format!("Para encerrar, pressione {STOP_HOTKEY}"))
                    .color(palette.muted)
                    .size(12.0),
            );
...
fn gravando(app: &mut App, ui: &mut egui::Ui, palette: &theme::Palette) {
    ui.label(RichText::new("● Gravando").size(24.0).strong().color(palette.accent));
    ui.add_space(8.0);
    ui.label(
        RichText::new(format!("{} passo(s) capturado(s)", app.recorded_steps))
            .size(16.0)
            .color(palette.muted),
    );
    ui.add_space(24.0);

    let botao = egui::Button::new(RichText::new("Encerrar gravação").size(16.0).strong())
    ...
    ui.add_space(8.0);
    ui.label(
        RichText::new(format!("ou pressione {STOP_HOTKEY} de qualquer lugar"))
            .color(palette.muted)
            .size(12.0),
    );
}
